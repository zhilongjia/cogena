...
enrichment analysis of up or down regulated genes as well as co-expressed 
genes resulting from various clustering methods. And the gene sets could 
be Pathway, Gene Ontology or user-defined gene-sets. Accordingly, this tool
can be used for pathway enrichment, GO enrichment and so on. See the workflow
...
1.  Make Annotation of gene set
    ...
    the gene-sets can be user-defined gene-sets formatted gmt, too (Just 
...
score of certain clustering methods and certain number of cluster.
    ...
    gene sets enriched in a local range of number of clusters. We consider 
...
2.  The heatmap of expression profiling with clusters is plotted by 
    `heatmapCluster`. see Figure 2. User can know which cluster contains
...
#The values shown in Figure 2. is the -log2 (adjusted p-values).
#Always make the number as character, please!
# The enrichment score for 6 clusters, together with I, II and All 
# (Down-regulated, Up-regualted and All DE genes)
...
# The probability to obtain such a number of interactions by chance considering PPI.
# PPIenrichment(clen_res, "hierarchical", "6", "2")

# Protein-protein interaction graph for a certain cluster. 
# plotPPI cannot work if there are more than 400 genes in a cluster.
# plotPPI(clen_res, "hierarchical", "6", "1")
```
