...
1.  Gene sets
    ...
    There are a variety of gene sets in the `cogena` package, partly collected from 
    ...
    the gene-sets can be user-defined gene-sets formatted gmt and compressed by xz, too. 
...
# KEGG Pathway Analysis as an example
...
#
# Always make the number as character, please!
heatmapPEI(clen_res, "pam", "7", printGS=FALSE)

#ordered by the Cluster 1.
#heatmapPEI(cogena_result, "pam", "7", orderMethod = "4")
```
4. Drug repositioning based on the pathway analysis results
```{r drp}
# A comprehensive way
...
# Showing the results ordered by the 4th cluster
heatmapPEI(cmapDn100_cogena_result, "pam", "7", printGS=FALSE, 
           orderMethod = "4", maintitle="Psoriasis")
```

```{r drp_figure2, fig.width=10, fig.height=6, fig.cap="Drug repositioning (merged instances)"}
# Showing the results ordered by the 4th cluster
heatmapCmap(cmapDn100_cogena_result, "pam", "7", printGS=FALSE, 
            orderMethod = "4", maintitle="Psoriasis")
```

5.  Genes in a certain cluster. User can obtain the genes in 
...
6.  The gene expression profilings with cluster infromation could be 
...
7. The correlation among one cluster could be shown via `corInCluster`. 
    see Figure 6.
