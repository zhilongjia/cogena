...
#' the largest Diversity and splits it into two nodes, called Cells. This process 
#' can be stopped at any level, assuring a fixed number of hard clusters. This 
#' behavior is achieved with setting the unrest.growth parameter to TRUE. Growth 
#' of the tree can be stopped based on other criteria, like the allowed maximum 
...
#' @param wcell alue specifying the winning cell migration weight. The default is 0.01.
#' @param pcell value specifying the parent cell migration weight. The default is 0.005.
#' @param scell value specifying the sister cell migration weight. The default is 0.001.
#' @param delta value specifying the minimum epoch error improvement. This value 
#' is used as a threshold for signaling the start of a new cycle. It is set to 1e-04 by default.
#' @param neighb.level integer value used to indicate which cells are candidates 
#' to accept new profiles. This number specifies the number of levels up the tree 
#' the algorithm moves in the search of candidate cells for the redistribution of 
...
#' @param unrest.growth logical flag: if TRUE then the algorithm will run maxCycles 
#' iterations regardless of whether the maxDiversity criteria is satisfied or not 
#' and maxCycles+1 clusters will be produced; if FALSE then the algorithm can 
#' potentially stop before reaching the maxCycles based on the current state of 
#' cluster diversities. A smaller than usual number of clusters will be obtained. 
...
#' nodes (cells). Node ID, its Ancestor, and 1's for a cell indicator are listed 
#' in the first three columns. Node profiles are shown in the remaining columns.}
...
sota <- function(data, maxCycles, maxEpochs=1000, distance="euclidean",
    ...
    out <- list(data=data, c.tree=cbind(tree[1:n,],Diversity=diversity), tree=treel, cluster=clust,
